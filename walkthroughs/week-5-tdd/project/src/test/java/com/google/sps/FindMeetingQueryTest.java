...
public final class FindMeetingQueryTest {
  ...
  public void onlyOptionalAttendees() {
    // Optional attendees with some events scheduled. Return all possible slots
    //
    // Optional:       |--A--|     |--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void onlyOptionalAttendeesButNoGaps() {
    // Only optional attendees but they have no gaps in their schedules.
    //
    //
    // Optional: |------A-----||-------B-------|
    // Day     : |-----------------------------|
    ...
    Collection<Event> events =
        Arrays.asList(
            ...
            new Event(
                "Event 2",
                TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY, false),
                Arrays.asList(PERSON_B)));
  ...
  @Test
  public void slotsIfCanAccomodateOptionalAttendee() {
    // If one or more time slots exists so that both mandatory and
  ...
  @Test
  public void overlappingWithOptionalAttendeesEvents() {
    // Events of mandatory attendee overlap with that of optional attendee
    // Return the overlapping bits of the free slots if they are long enough
    //
    // Optional:        |----C------|
    // Events  :     |--A--|     |--B--|
    // Day     : |-----------------------------|
    ...
    Collection<Event> events =
        Arrays.asList(
            ...
            new Event(
                "Event 2",
                TimeRange.fromStartDuration(TIME_0845AM, DURATION_60_MINUTES),
                Arrays.asList(PERSON_C)),
             new Event(
                "Event 3",
  ...
  @Test
  public void freeOverlappingSlotsTooSmall() {
    // The free slots for mandatory and optional attendees overlap, but most
    // overlaps are too small. Only keep overlapping bit that is big enough
    // to accommodate meeting
    //
    // Optional:         |-B-|        |D|
    // Events  : |--A--|      |--C--|
    // Day     : |------------------------------|
    ...
    Collection<Event> events =
        Arrays.asList(
            ...
            new Event(
                "Event 2",
                TimeRange.fromStartDuration(TIME_0845AM, DURATION_30_MINUTES),
                Arrays.asList(PERSON_B)),
             new Event(
                "Event 3",
            ...
            new Event(
                "Event 4",
                TimeRange.fromStartDuration(TIME_1045AM, DURATION_15_MINUTES),
  ...
  @Test
  public void justEnoughRoomIgnoreOptionalAttendee() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting. An optional attendee has an event which would leave too little time
    // for the meeting if it were taken into consideration.
    //
    // Optional:       |-B-|
    // Events  : |--A--|     |----A----|
    // Day     : |---------------------|
